                              ...
                              const u_char *packet)
{
  ...
  if (ntohs(eth->ether_type) == 0x0800) { // 0x0800 is IP type
    ...
    unsigned short ipHeaderLen = ip->iph_ihl * 4;
    ...
    for(int i = 0; i < data_length; i++){
      printf("%02x ", packet[sizeof(struct ethheader) + sizeof(struct ipheader) + sizeof(struct tcpheader) + i]);
    }
